---
layout: post
date: 2026-05-06
title: "The Indentation Project, Part 2: Surfaces, Storage, and a Drawer That Wouldn't Open"
excerpt: "After ver1, the room had a frame and a sofa, but no surfaces and no drawers. ver2 turned a structure into something usable — and taught me that a perfect fit can be a defect."
comments: true
lang: en
lang-ref: indentation-part-2
genre: practical
thumbnail: /assets/images/indentation/IMG_3664.jpg
---

*This continues the Indentation Project series. The build itself happened a while ago; I'm writing it up now. [Part 1](/en/2026/04/09/The-Indentation-Project-Part-1/) covered the design and the wall-to-ceiling frame.*

## Where ver1 left things

ver1 ended with a structure but not yet furniture. The wall-to-ceiling frame was up. The sofa was sitting on its raised platform. The vertical sections were anchored, the horizontal beams were in. From across the room it already looked finished.

It wasn't. The shelves had no shelf surfaces — just empty rectangles where boards should be. The space under the sofa was a cavity, not a drawer. You couldn't put a book down anywhere. The frame had earned the right to exist, but until I added top surfaces and a drawer, the room had a sculpture, not a workspace.

ver2 was the part that turned it into something we could use.

## Choosing the top board material

The top boards had a constraint I hadn't thought hard about during the frame design: they couldn't be heavy.

Several of the shelves are cantilevered — supported only on one or two sides — to keep the indentation cavities open and visually light. A heavy top board on a cantilevered span shifts load onto the joints I least want to load. The top board needed to hold books and craft objects without flexing or cracking, but it also had to add as little weight as possible.

The default options pushed back. Plywood is cheap and stiff, but heavier than it looks once you cover ten meters of shelving. Solid oak or pine boards thick enough to span the width without sagging would have weighed even more. Thin MDF would crack the first time someone leaned on it.

I went with paulownia. It's the lightest commercially available wood I could buy in board form — used traditionally for tansu and storage chests precisely because it's light, dimensionally stable, and surprisingly durable. It's also cheap. The trade-off is that paulownia boards are thin. If I just laid them on the frame, they'd flex under a stack of books.

The fix was a sub-frame: scraps of 2x2 left over from the ver1 build, screwed into a grid that matched each shelf's footprint, with the paulownia laid on top. The 2x2s carry the load; the paulownia provides the surface. Light, strong, cheap.

No single paulownia board was long enough for any of the shelves, so each surface is multiple boards arranged side by side. The cuts at each corner had to wrap around the vertical 2x2s of the frame — I'd draw a line, cut with a saw, dry-fit, sand, dry-fit again, sand again. Iterative fitting. Once a board was right, I clamped it down with wood glue and a small amount of instant adhesive at the corners so it wouldn't shift while the glue set. About ten boards in total. Repetitive, but the rhythm was satisfying.

## Building the under-sofa drawer

With the surfaces in, the room could finally be used. But the sofa platform still hid a cavity that was just sitting there — the largest single piece of usable storage in the apartment, and at that point, completely empty.

The drawer design was simple. A rectangular frame, slightly narrower than the cavity (about a centimeter of clearance), with a bottom panel of three boards, casters underneath, and a face board attached at the front.

Two material choices saved money. First, the drawer frame is built almost entirely from leftover 2x2 from ver1. When I'd designed the frame as "smallest reusable units," I'd been thinking about disassembly — but the same modular sizing meant the offcuts came out in lengths that matched the drawer's interior dimensions almost exactly. I didn't plan for this. It was a free win.

Second, I bought 2x4 from Cainz for the parts that needed more depth, and the lumber was visibly warped. Normally that's a return-it problem. Here, the warp didn't matter: the drawer slides on casters, so no lateral force is ever applied to the frame. The warp was a cosmetic defect on a part nobody would see, doing a job that didn't care about it. I overlooked it and moved on.

## What went wrong: the drawer wouldn't open

I finished the drawer, slid it into the cavity, and stepped back. It looked great. The face board sat flush with the frame around it. The casters rolled smoothly. The fit was perfect.

It was too perfect.

There was no way to pull it out. The face was flush. There was no lip, no protrusion, no gap to hook a finger into. I'd designed the clearance for the drawer's *body* — about a centimeter on each side so it wouldn't bind — but I'd given zero thought to how a hand was supposed to grab the *face*. The drawer worked mechanically and was operationally useless.

The obvious fix was a handle. I almost installed one — and then I sat on the sofa above the drawer and realized: my feet would land within a few centimeters of where any front-mounted handle would protrude. Every time someone sat down, their heels would catch on the handle. Worse, the kick would apply upward force, pulling the handle away from the face and eventually loosening the screws.

A handle on the inside of the face would solve both problems — no protrusion, nothing for feet to hit, and a finger-pull recessed enough to be invisible from a normal viewing angle. So I drilled a hole through the face. Not too large, not too small. Big enough for two fingers, small enough that it doesn't read as a hole when you look at the drawer head-on.

It works. After varnishing the whole assembly, the drawer disappears into the platform, and the only way you'd know it's there is if you crouch down and see the recessed grip.

## What I learned

**Material choice is constraint-routing.** Paulownia wasn't the obvious answer. Plywood would have been cheaper and easier to source; oak would have been more visually impressive. But the actual constraint — light enough not to load the cantilevers, durable enough for daily use, cheap enough to cover ten meters — only paulownia satisfied. The "right" material is rarely the most familiar one. You find it by writing down the constraints and crossing things off.

**Reusing scrap closes a design loop.** The ver1 decision to build verticals as small modular units paid off twice. Once when I designed for disassembly. Again when the offcuts turned out to be sized exactly for ver2's drawer frame. I didn't plan the second payoff, but it wouldn't have happened with longer, less repetitive lumber. Modular design propagates downstream in ways the original design didn't intend.

**A perfect fit can be a defect.** The drawer's body fit the cavity with the right tolerance. The drawer's face fit the frame with no tolerance at all — and that meant there was no way to grab it. Static fit and operational fit are two different things, and if you only design for the first, you ship furniture that works exactly once: when you put it together. Designing clearance *for use* — for the hand that grabs, the foot that kicks, the finger that pulls — is a separate pass. I'd skipped it.

After ver2, the frame finally felt like furniture. The shelves had surfaces. Books and objects had homes. The drawer held everything that used to live in plastic bins on the floor. The room still needed lighting — the ceiling fixture lit the whole apartment but didn't put any light where it was actually needed, on the desk surfaces and the sofa table — but that was a problem for ver3.

---

*[Part 1](/en/2026/04/09/The-Indentation-Project-Part-1/) covers the design and frame construction. The full photo gallery for ver2 is available [here](/en/2025/01/20/The-indentation-Project-ver2/).*
